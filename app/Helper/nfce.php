...
function inserirNfcePelaComanda($VendaComanda, $natureza_operacao, $tributacao){
    $emitente           = Emitente::where("empresa_id", $VendaComanda->empresa_id)->first();
    ...
    $nota->empresa_id   = $VendaComanda->empresa_id;
    $nota->comanda_id  = $VendaComanda->id;
    $nota->modelo       = config("constanteNota.mod.NFCE");
    ...
    $nota->desconto_nota= 0.00; //$VendaComanda->valor_desconto;
    $nota->vDesc        = 0.00; //$VendaComanda->valor_desconto;
    $nota->vOutro       = 0.00; //$VendaComanda->valor_acrescimo;
    ...
    //Destinatário
    $nota->cliente_cpf  = null; //  ($VendaComanda->cliente_cpf) ? tira_mascara($VendaComanda->cliente_cpf) : null;
    $nota->cliente_cnpj = null; // ($VendaComanda->cliente_cnpj) ? tiraAcento($VendaComanda->cliente_cnpj) : null;
    $nota->indIEDest    = null; // ($VendaComanda->cliente_cpf) ? 9 : null;

    $nfce               = Nfce::where("pdvvenda_id", $VendaComanda->id)->first();
    if(!$nfce){
        $nota->serie        = $emitente->numero_serie_nfce;
        $nota->nNF          = $emitente->ultimo_numero_nfce + 1;
        $nota->cNF          = rand($nota->nNF,99999999);
    ...
    NfceItem::where("nfce_id", $id_nfce)->delete();
    foreach($VendaComanda->itens as $i){
        $item                  = new \stdClass();
        $item->nfce_id         = $id_nfce;
        ...
        //$item->subtotal_liquido= $i->subtotal_liquido;
        $item->desconto_por_unidade     = 0.00; //$i->desconto_por_unidade;
        $item->total_desconto_item      = 0.00; //$i->total_desconto_item;
        $item->desconto_percentual      = 0.00; //$i->desconto_percentual;
        $item->desconto_por_valor       = 0.00; //$i->desconto_por_valor;
        $item->subtotal_liquido         = $i->valor ;
    ...
    //Duplicata
    NfceDuplicata::where("nfce_id", $id_nfce)->delete();
    if(count($VendaComanda->duplicatas) > 0){
        $contFatura = 1;
        foreach($VendaComanda->duplicatas as $ft){
            $dup        = new \stdClass();
            $dup->nfce_id = $id_nfce;
            $dup->nDup  = 1; //$ft->nDup;
            $dup->dVenc = hoje(); //$ft->dVenc;
            $dup->vDup  = $VendaComanda->total;
            $dup->tPag  = '01'; //$ft->tPag;
            $dup->obs   = null; //$ft->obs;
            NfceDuplicata::Create(objToArray($dup));
            $contFatura++;
